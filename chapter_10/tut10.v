...
/*Produced by NSL Core(version=20250428), IP ARCH, Inc. Fri Apr 17 20:59:02 2026
...
always @(posedge m_clock) // there are several "always @(posedge m_clock)" blocks in this file.
  begin                   // It is because we want to divide the circuit as a function. 
    if((_divid_divu_error != 1'b0))   // If we put all the statements in one "always @(posedge m_clock)" block, 
    begin                             // the circuit will be very complicated and it becomes difficult to optimize the circuit. // YOSHII memo
    $display("divid error");
    end
  end
always @(posedge m_clock)
